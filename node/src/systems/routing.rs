...
/// Traversal Packet Receiver
#[derive(Resource)]
pub struct TraversalPacketReceiver {
	receiver: thingbuf::mpsc::blocking::Receiver<Vec<u8>>
}
	...
	for packet in packets.iter() {
		...
		if packet.recipient == config.node_id {
			if let Some(entity) = id_map.map.get(&packet.recipient) {
				let packet = rkyv::check_archived_root::<'_, NodePacket<Net>>(&packet.packet).expect("failed to unarchive traversed packet");
			...
			if let Some((sess, _)) = peers.iter()
				...
				sess.send_packet(NodePacket::Traversal { destination: packet.destination, encrypted_packet: packet.packet.clone() })
			}
		}
	}
}