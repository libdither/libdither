...
use futures::{
    channel::mpsc::{self, unbounded, TrySendError, UnboundedSender},
    StreamExt,
};
...
impl<Net: Network> Node<Net> {
    pub fn new(config: NodeConfig<Net>, event_sender: UnboundedSender<NodeEvent<Net>>) -> Self {
        ...
        world.insert_resource::<EventSender<Net>>(EventSender {
            sender: event_sender,
        });
    ...
    pub async fn run(
        mut self,
        mut action_receiver: mpsc::UnboundedReceiver<NodeAction<Net>>,
    ) -> Result<Self, Net::ConnectionError> {
        let config = self.world.resource::<NodeConfig<Net>>().clone();

        log::info!("listener config: {:?}", config.listener_config);

        let (network, mut connection_stream) =
            Net::init(config.keys.clone(), &config.listener_config).await?;
        self.world
            .insert_resource::<Net::ListenerConfig>(config.listener_config.clone());
        ...
        let (entity_event_sender, mut entity_event_receiver) =
            unbounded::<EntitySessionEvent<Net>>();
    ...
    fn handle_session_events(world: &mut World, session_event: EntitySessionEvent<Net>) {
        ...
        match event {
            SessionEvent::Packet(packet) => match packet {
                NodePacket::DiscoveryPacket(packet) => {
                    DiscoverySystem::handle_packet(world, entity, packet)
                }
            ...
            SessionEvent::Traversal(packet) => {
                if let Some((sess, _)) = world
                    .query::<(&Session<Net>, &Coordinates)>()
                    .iter(&world)
    ...
    async fn handle_node_action(&mut self, action: NodeAction<Net>) -> Result<(), NodeError<Net>> {
        match action {
            NodeAction::Connect(remote_id, remote_addr, pub_key) => {
                ...
                let (pub_key, persistent_state) = if let Some(entity) = entity {
                    ...
                    if self.world.get::<Session<Net>>(entity).is_some() {
                        log::info!(
                            "NodeAction: Connect: Already Connected to Remote: {remote_id:?}"
                        );
                        return Ok(());
                    }

                    // Check if Session exists and if so, also if the pub_key matches.
                    let persistent_state = if let Some(session) =
                        self.world.get::<SessionInfo<Net>>(entity)
                    {
                    ...
                    let entity = self
                        ...
                        .id();
                    self.world
                        .resource_mut::<RemoteIDMap>()
                        .map
                        .insert(remote_id.clone(), entity);

                    (pub_key, None)
                };
                // Connect to it via Network
                self.world.resource::<Net>().connect(
                    remote_id,
                    remote_addr,
                    pub_key,
                    persistent_state,
            ...
            NodeAction::GetRemoteInfo(entity) => {
                ...
                if let Ok((entity, remote, latency_metrics, coords)) = self
                    .world
                    .query::<(Entity, &Remote, &LatencyMetrics, Option<&Coordinates>)>()
                    .get(&self.world, entity)
                {
                    ...
                    log::error!(
                        "entity {entity:?} exists but has components: {:?}",
                        self.world
    ...
    fn handle_connection(
        ...
        session_event_sender: UnboundedSender<EntitySessionEvent<Net>>,
    ) {
        // Derive remote ID
        let remote_id = NodeID::hash(connection.remote_pub_key.as_ref());

        log::info!(
            "received connection from {remote_id:?} from address: {:?}",
            connection.incoming_address
        );
        ...
        let shared = self
            .world
            .resource::<SharedSessionState<Net>>()
            .state
            .clone();
...
fn check_closed_session<Net: Network>(
    mut commands: Commands,
    sessions: Query<(Entity, &Session<Net>)>,
) {
