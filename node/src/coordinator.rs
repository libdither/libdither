...
impl<Coord: CoordinateType, const ADAPT_CONST: f64, const ERROR_CONST: f64> NodeCoordinator for AdaptiveCoordinator<Coord, ADAPT_DELTA_CONST, ERROR_CONST> {
    ...
    fn update(&mut self, remotes: impl Iterator<Item = &mut RemoteState>) {
        for remote in remotes {
            ...
            let mut displacement = self.coord.clone().subtract(remote.coord);
            
            // Estimate of distance of the remote node
            let distance_estimate = displacement.length();
            ...
            let relative_sample_error = sample_error / remote.measured_distance;
            ...
            let sample_error_weight = self.error / (self.error + remote.error);

            // Update moving average of this node's error
            self.error = relative_sample_error * ERROR_CONST * sample_error_weight + self.error * (1 - ERROR_CONST * sample_error_weight);

            // Timestep scales with error
            let delta = ADAPT_CONST * sample_error_weight;
            // Move coord
