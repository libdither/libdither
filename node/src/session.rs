...
use rkyv::{option::ArchivedOption, Archived, Deserialize, Infallible};
use smallvec::SmallVec;
use thiserror::Error;

use crate::{
    packet::{PacketRead, PacketWrite},
    ArchivedNodePacket, Connection, Network, NodeID, NodePacket, PingingNodePacket,
    TraversalPacket,
};
...
impl<Net: Network> Session<Net> {
    pub fn spawn(
        ...
        task::spawn(async move {
            if let Err(err) = SessionState::run(
                connection,
                shared,
                entity_id,
    ...
    pub fn send_action(&self, action: SessionAction<Net>) {
        if let Err(err) = self.action_sender.unbounded_send(action) {
            log::warn!(
                "Tried to send SessionAction: {:?} but session was closed",
                err
            );
        }
    }
    pub fn send_packet(&self, packet: NodePacket<Net>) {
        if let Err(err) = self
            .action_sender
            .unbounded_send(SessionAction::Packet(packet))
        {
...
impl<Net: Network> SessionState<Net> {
    ...
    async fn run(
        ...
        entity_id: Entity,
        event_sender: UnboundedSender<EntitySessionEvent<Net>>,
        mut action_receiver: UnboundedReceiver<SessionAction<Net>>,
    ) -> Result<(), SessionError<Net>> {
    ...
    pub async fn handle_ping_packet(
        &mut self,
        pinging_packet: &Archived<PingingNodePacket<Net>>,
    ) -> Result<(), SessionError<Net>> {
        // Record acknowledged ping
        if let Some(ack) = pinging_packet
            .ack_ping
            ...
            .unwrap()
        {
            match self.ping_tracker.record_unique_id(ack) {
                Ok(duration) => {
                    ...
                    self.event_sender.unbounded_send(EntitySessionEvent {
                        entity: self.entity_id,
                        event: SessionEvent::LatencyMeasurement(duration),
                    })?;
                }
                Err(err) => {
                    log::warn!(
                        "session: ping tracker: error when recording acknowledged ping id: {err}"
                    );
    ...
    pub async fn handle_packet(
        &mut self,
        packet: &Archived<NodePacket<Net>>,
    ) -> Result<(), SessionError<Net>> {
        match packet {
            ...
            ArchivedNodePacket::Traversal(packet) => {
                ...
                self.event_sender
                    .send(EntitySessionEvent {
                        entity: self.entity_id,
                        event: SessionEvent::Traversal(traversal_packet),
                    })
                    .await?;
            }
            packet => {
    ...
    pub async fn handle_session_action(
        ...
        match action {
            ...
            SessionAction::Ping(ping_count) => {
                ...
                log::debug!(
                    "{:?} SessionAction::Ping: {ping_count:?}, countdown: {:?}",
                    self.entity_id,
                    self.ping_countdown
                );
                ...
                if self.ping_countdown != 0 {
                    ...
                    let ping_id =
                        (self.ping_countdown != 0).then(|| self.ping_tracker.gen_unique_id());
                    let packet = PingingNodePacket::<Net> {
                        packet: None,
                        ping_id,
                        ack_ping: None,
                    };
...
const MAX_PENDING: u8 = 64;

/// High-performance Ping Tracker
#[derive(Default, Debug, Clone)]
struct PingTracker {
    // Slotmap-like statically-sized queue for maximum performance! :D
    ...
    ping_queue: SmallVec<(PingSlot, u8), { MAX_PENDING as usize }>,
