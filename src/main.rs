...
use node::{EncryptionKeys, Network, Node, NodeAction, NodeConfig, NodeID, NodePacket};
...
async fn main() -> anyhow::Result<()> {
    ...
    let listen_port: u16 = match std::env::args().nth(1).map(|s| s.parse()) {
        ...
        None => {
            return Ok(println!(
                "Requires a port number as a command line argument"
            ))
        }
...
async fn handle_command(
    line: String,
    action_sender: &mut mpsc::UnboundedSender<NodeAction<DitherNet>>,
    stdout: &mut SharedWriter,
) -> anyhow::Result<()> {
    let mut split = line.split(" ");
    let line = if let Some(split) = split.next() {
        split
    } else {
        return Ok(());
    };
    match line {
        "connect" => {
            let node_id = split
                ...
                .ok_or(anyhow!("Failed to parse NodeID"))??;
            let addr = split
                .next()
                .map(|s| s.parse::<Address>())
                .ok_or(anyhow!("Failed to parse Multiaddr"))??;
            action_sender
                .send(NodeAction::Connect(node_id.clone(), addr, None))
                .await?;
        ...
        "info" => {
            let dec = text::int::<_, Simple<char>>(10).try_map(|s, span| {
                s.parse::<u32>()
            ...
            let parser = dec
                ...
                .map(|(index, generation)| {
                    Entity::from_bits(index as u64 & ((generation as u64) << 32))
                });

            let to_parse = split.next().ok_or(anyhow!(
                "must pass an Entity ID of format: <index>v<generation>"
            ))?;

            let entity = parser
                .parse(to_parse)
                .map_err(|err| anyhow!("parsing error: {}", err[0]))?;
            log::debug!("Getting info of entity: {:?}", entity);
            action_sender
                .send(NodeAction::GetRemoteInfo(entity))
                .await?;
        }
        "print" => {
        ...
        "data" => {
            ...
            let data = split
                .remainder()
                .ok_or(anyhow!("Data not passed"))?
                .as_bytes()
                .to_vec();

            action_sender
                .send(NodeAction::ForwardPacket(node_id, NodePacket::Data(data)))
                .await?;
        }
        "help" => writeln!(
            stdout,
...
data <NodeID> <String> - send arbitrary data to another node
        ...
        _ => {
            writeln!(stdout, "Unknown command, type help for a list of commands")?;
        }
    }
    Ok(())
}
